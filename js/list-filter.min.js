...
b('input[name^="value"]').PDb_clearInputs("");m();d.listpage="1";break;case "page":d.action=c.closest("div").data("action");d.listpage=c.data("page");break;case "sort":break;default:return}e?"clear"!==d.submit&&(a.closest("form").append(b("<input>",{type:"hidden",name:"submit",value:a.val()})),a.closest("form").submit()):(a.PDb_processSubmission(),b("html").trigger("pdbListFilterComplete"))},r=function(a){g(a,!0)},t=function(a){b(a.target).data("submit","page");var e=b(a.target).closest(".wrap.pdb-list").prop("class").match(/pdb-instance-(\d+)/);
