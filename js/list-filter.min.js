...
e:b(".pdb-list").first(),g=c.find(".pdb-pagination"),f=a.closest("fieldset, div"),k=b(PDb_ajax.loading_indicator).clone();b.ajax({type:"POST",url:PDb_ajax.ajaxurl,data:d,beforeSend:function(){g.find("a").prop("disabled",!0);f.append(k)},success:function(e,h){if(/^failed/.test(e))switch(d.submit){case "page":h=document.createElement("a");h.href=window.location.href;window.location.href=h.protocol+"//"+h.hostname+a.attr("href");break;default:l.append('<input type="hidden" name="submit_button" value="'+
